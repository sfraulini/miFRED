...
def validate(classes, ko_validation, ko_aceto):
    results_per_class = {}
    scores = {}
    all_functions_list = []
...
if __name__ == '__main__':
    ...
    if not args.all_genomes:
        ...
        if os.path.exists(os.path.join(args.output_folder, 'input_fred')) == False:
            try:
                os.mkdir(os.path.join(args.output_folder, 'input_fred'))
                print(f"Directory '{os.path.join(args.output_folder, 'input_fred')}' created successfully.")
            except PermissionError:
                print(f"Permission denied: Unable to create '{os.path.join(args.output_folder, 'input_fred')}'.")
        else:
            print(f"Directory '{os.path.join(args.output_folder, 'input_fred')}' already exists.")
