...
for f in $(ls *"_R1"* 2> /dev/null | grep -v '_unpaired'); do
    ...
    if [ $unpaired == 'False' ]; then
        ...
        bowtie2 -q -1 $f -2 $r -x $output_dir/input_fred/bowtie2/index -p $proc -S $output_dir/input_fred/bowtie2/$sample.sam >> "$output_dir/logs/bowtie2.log" 2>&1
    else
        ...
        bowtie2 -q -1 $f -2 $r -U $uf,$ur -x $output_dir/input_fred/bowtie2/index -p $proc -S $output_dir/input_fred/bowtie2/$sample.sam >> "$output_dir/logs/bowtie2.log" 2>&1
    fi
done 
